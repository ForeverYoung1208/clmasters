      ...
      <Form onSubmit={submitHandler} className="preorder-form">
        ...
        <input
          className={`form-input ${
            validationErrors?.name && 'form-input--invalid'
          }`}
          type="text"
          name="name"
          id="name"
        ...
        />
        <div className="preorder-form__validation-error">
          {validationErrors?.name || <Emptyspace />}
        </div>

        <label className="preorder-form__form-label" htmlFor="email">
          * Enter your email:
        </label>
        <input
          className={`form-input ${
            validationErrors?.email && 'form-input--invalid'
          }`}
          type="email"
          name="email"
          id="email"
        ...
        <div className="preorder-form__validation-error">
          {validationErrors?.email || <Emptyspace />}
        </div>

        <label className="preorder-form__form-label" htmlFor="clockTypeId">
          * Clock size:
        </label>
        <select
          className={`form-select ${
            validationErrors?.clockTypeId && 'form-input--invalid'
          }`}
          name="clockTypeId"
          id="clockType"
          ...
          {clocks?.map((c) => (
            <option key={c.id} value={c.id}>
              ...
              {c.id === -1
                ...
                : `, repair time: ${
                    vocabluaries.clocks?.find((cl) => cl.id === c.id)
                      ?.repairTime || <Emptyspace />
                  }`}
            </option>
          ))}
        </select>
        <div className="preorder-form__validation-error">
          {validationErrors?.clockTypeId || <Emptyspace />}
        </div>

        <label className="preorder-form__form-label" htmlFor="cityId">
          * City:
        </label>
        <select
          className={`form-select ${
            validationErrors?.cityId && 'form-input--invalid'
          }`}
          name="cityId"
          id="city"
        ...
        <div className="preorder-form__validation-error">
          {validationErrors?.cityId || <Emptyspace />}
        </div>
        <label htmlFor="Date">* Desired date and time:</label>
        <DatePicker
          className={`form-input ${
            validationErrors?.orderDateTime && 'form-input--invalid'
          }`}
        ...
        />
        <div className="preorder-form__validation-error">
          {validationErrors?.orderDateTime}
        </div>

        <Button
          type="submit"
          disabled={isLoading || !validationErrors?.isAllValid}
        >
          Submit information
        </Button>
      </Form>
